...
from uer.utils.tokenizer import SpacyTokenizer
...
class BertClassifier(nn.Module):
    def __init__(self, args, model):
        ...
        if self.if_lstm is True:
            ...
            self.fc = nn.Linear(args.hidden_size * 2, args.labels_num)
        else:
    ...
    def forward(self, src, postag_ids, label, mask, pos=None, vm=None):
        ...
        if self.args.encoder == "bert":
            if not self.args.use_baseline_bert:
                emb = self.embedding(src, postag_ids, mask, pos)
            else:
                # 在使用 baseline bert 时，直接返回 loss
                return self.encoder(src, labels=label)
                # logits =
        ...
        # 输入到 bilstm
        if self.if_lstm is True:
            out, (hidden_last, cn_last) = self.lstm(output)
            # 双向的需要单独处理
...
def add_knowledge_worker(params, postag_dict):
    ...
    if args.use_baseline_bert:
        ...
        tokenizer = BertTokenizer.from_pretrained(args.baseline_bert_path)
    ...
    for line_id, line in enumerate(sentences):
        ...
        if args.use_baseline_bert:
            token_ids = tokenizer.encode(line[columns["text"]], add_special_tokens=True, max_length=args.seq_length,
            ...
            tokens, pos, vm, _ = kg.add_knowledge_with_vm([text], add_pad=True, max_length=args.seq_length,
                                                          use_bert_tokenizer=args.use_baseline_bert)
            tokens = tokens[0]
            pos = pos[0]
            vm = vm[0].astype("bool")

            token_ids = [vocab.get(t) for t in
                         tokens] if not args.use_baseline_bert else tokenizer.convert_tokens_to_ids(
                tokens)
            # decoded_tokens = tokenizer.convert_ids_to_tokens(token_ids) if args.use_baseline_bert else tokens
...
def main():
    ...
    parser.add_argument("--dataset_path", default="./data/plm+kg-hate-corpus-v1/stg1/")
    parser.add_argument("--train_path", type=str, required=False,
                        default="./data/plm+kg-hate-corpus-v1/stg1/train.tsv",
                        help="Path of the trainset.")
    parser.add_argument("--dev_path", type=str, required=False, default="./data/plm+kg-hate-corpus-v1/stg1/dev.tsv",
                        help="Path of the devset.")
    parser.add_argument("--test_path", type=str, required=False, default="./data/plm+kg-hate-corpus-v1/stg1/test.tsv",
    ...
    parser.add_argument("--use_baseline_bert", action="store_true", help="Use baseline bert.")
    parser.add_argument("--baseline_bert_path", type=str, default="./bert-base-uncased",
                        help="Path of the baseline bert.")
    ...
    # 实验baseline中的bert模型必须保证使用的tokenizer是bert
    assert not args.use_baseline_bert or args.tokenizer == "bert", "The tokenizer must be bert when using baseline bert."
    ...
    if not args.use_baseline_bert:
        ...
        # 加载bert词表
        from transformers import BertTokenizer
        args.vocab = BertTokenizer.from_pretrained(args.baseline_bert_path)
    ...
    def batch_loader(batch_size, input_ids, postag_ids, label_ids, mask_ids, pos_ids, vms):
        ...
        for i in range(instances_num // batch_size):
            ...
            mask_ids_batch = mask_ids[i * batch_size: (i + 1) * batch_size, :] if not args.use_baseline_bert else None
            pos_ids_batch = pos_ids[i * batch_size: (i + 1) * batch_size, :] if not args.use_baseline_bert else None
            vms_batch = vms[i * batch_size: (i + 1) * batch_size] if not args.use_baseline_bert else None
        ...
        if instances_num > instances_num // batch_size * batch_size:
            ...
            mask_ids_batch = mask_ids[instances_num // batch_size * batch_size:,
                             :] if not args.use_baseline_bert else None
            pos_ids_batch = pos_ids[instances_num // batch_size * batch_size:,
                            :] if not args.use_baseline_bert else None
            vms_batch = vms[instances_num // batch_size * batch_size:] if not args.use_baseline_bert else None
    ...
    def evaluate(args, is_test, metrics='Acc'):
        ...
        if args.use_postag:
            ...
            mask_ids = torch.LongTensor([sample[2] for sample in dataset]) if not args.use_baseline_bert else None
            pos_ids = torch.LongTensor([example[3] for example in dataset]) if not args.use_baseline_bert else None
            vms = [example[4] for example in dataset] if not args.use_baseline_bert else None
        ...
        for i, (input_ids_batch, postag_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch,
            ...
            vms_batch = torch.LongTensor(vms_batch) if not args.use_baseline_bert else None

            input_ids_batch = input_ids_batch.to(device)
            postag_ids_batch = postag_ids_batch.to(device) if postag_ids_batch is not None else None
            label_ids_batch = label_ids_batch.to(device)
            mask_ids_batch = mask_ids_batch.to(device) if not args.use_baseline_bert else None
            pos_ids_batch = pos_ids_batch.to(device) if not args.use_baseline_bert else None
            vms_batch = vms_batch.to(device) if not args.use_baseline_bert else None
    ...
    if args.use_postag:
        ...
        mask_ids = torch.LongTensor([example[2] for example in trainset]) if not args.use_baseline_bert else None
        pos_ids = torch.LongTensor([example[3] for example in trainset]) if not args.use_baseline_bert else None
        vms = [example[4] for example in trainset] if not args.use_baseline_bert else None
        postag_ids = None
    ...
    if not args.use_baseline_bert and args.encoder == 'bert':
